...
class bts(nn.Module):
    def __init__(self,  feat_out_channels, num_features=512):
        ...
        self.daspp_conv = torch.nn.Sequential(
            ...
            nn.ELU())
        self.reduc8x8 = reduction_1x1(num_features // 4, num_features // 4, 10)
        ...
        self.conv3 = torch.nn.Sequential(
            ...
            nn.ELU())
        self.reduc4x4 = reduction_1x1(num_features // 4, num_features // 8, 10)
        ...
        self.reduc2x2 = reduction_1x1(num_features // 8, num_features // 16, 10)
        self.lpg2x2 = local_planar_guidance(2)

        self.upconv1 = upconv(num_features // 8, num_features // 16)
        self.reduc1x1 = reduction_1x1(num_features // 16, num_features // 32, 10, is_final=True)
        self.conv1 = torch.nn.Sequential(nn.Conv2d(num_features // 16 + 4, num_features // 16, 3, 1, 1, bias=False),
                                         nn.ELU())
        self.get_depth = torch.nn.Sequential(nn.Conv2d(num_features // 16,3, 3, 1, 1, bias=False),
                                             # nn.UpsamplingBilinear2d(size=(256, 320)),
                                            )
        # scale 0
    ...
    def forward(self, features, focal):
        ...
        depth_8x8_scaled = depth_8x8.unsqueeze(1) / 10
        ...
        depth_4x4_scaled = depth_4x4.unsqueeze(1) / 10
        ...
        depth_2x2_scaled = depth_2x2.unsqueeze(1) / 10
        ...
        final_depth = 10 * self.get_depth(iconv1)
...
class BtsModel(nn.Module):
    def __init__(self, params):
        ...
        config_vit.n_skip = 3
        config_vit.patches.grid = (int(params.input_height / 16), int(params.input_width / 16))
